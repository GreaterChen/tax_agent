...
class NewsCrawlerAgent:
    ...
    def __init__(self, db_url: str, tongyi_api_key: str):
        """初始化爬虫 Agent
        
        Args:
            db_url: 数据库连接URL
            tongyi_api_key: AI API密钥
        """
...
{{
    ...
    def save_to_db(self, news_data: Dict, source: Dict):
        ...
        insert_query = text("""
            INSERT INTO news (language, source, date, content, url, title, tags)
...
def main():
    ...
    if not db_url:
        ...
        print("示例: export DATABASE_URL='sqlite:///news.db'")
        print("或: export DATABASE_URL='postgresql://user:password@localhost/dbname'")
        return
